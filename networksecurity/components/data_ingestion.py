...
class DataIngestion:
    def __init__(self,data_ingestion_config:DataIngestionConfig):
        try:
            ...
            logging.error(f"Error occurred while initializing DataIngestion class: {e}")
    ...
    def split_data_as_train_test(self,dataframe:pd.DataFrame):
        try:
            ...
            os.makedirs(dir_path,exist_ok=True)
            logging.info(f"Saving training set to file: {self.data_ingestion_config.training_file_path}")
            train_set.to_csv(self.data_ingestion_config.training_file_path,index=False,header=True)
            test_set.to_csv(self.data_ingestion_config.testing_file_path,index=False,header=True)
            logging.info(f"Saved training set and testing set successfully")
            #return train_set,test_set
        except Exception as e:
            logging.error(f"Error occurred while performing train test split: {e}")
            raise NetworkSecurityException(e,sys)
